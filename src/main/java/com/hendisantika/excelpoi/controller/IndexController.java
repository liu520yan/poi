...
public class IndexController {
    ...
    public void exportSelectResult(HttpServletResponse response ,@PathVariable(value = "count") int count) {

        long startTime = System.currentTimeMillis();
        List<BarCodeForm> lists = new ArrayList<>(count);
        for (int i = 0; i < count; i++) {
            lists.add(new BarCodeForm("sdsdwewe", "sdsdwewe", "sdsdwewe", "sdsdwewe", "sdsdwewe", "sdsdwewe", "sdsdwewe", "sdsdwewe", "sdsdwewe", "sdsdwewe"));
        }
        ...
        for (int i = 0; i <= cycle; i++) {
            ...
            if ( count - i * everySheet <= everySheet) {
                list = lists.subList(i * everySheet, count);
                System.out.println("1   起：" + i * everySheet + "         终" + count);
